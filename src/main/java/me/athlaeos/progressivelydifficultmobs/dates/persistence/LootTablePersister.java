package me.athlaeos.progressivelydifficultmobs.dates.persistence;
...
public class LootTablePersister {
    ...
    public static void deleteLootTable(LootTable table) {
        File file = new File(ProgressivelyMain.getInstance().getDataFolder() + File.separator + "loottables" + File.separator + table.getName() + ".yml");
        if (file.exists()) {
            file.delete();
    ...
    public static void loadLootTables() {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                try {
                    File lootTableDirectory = new File(ProgressivelyMain.getInstance().getDataFolder(), "loottables");
...
//                    if (lootTableDirectory.list() == null) return;
                        ...
                        if (section != null) {
                            for (String drop : section.getKeys(false)) {
                                ...
                                try {
                                    ...
                                    ProgressivelyMain.getInstance().getServer().getConsoleSender().sendMessage(Utils.chat("&cDrop type " + dropItem + " was not recognized for this version of minecraft"));
                                    ...
                                    ProgressivelyMain.getInstance().getServer().getConsoleSender().sendMessage(Utils.chat("&center the LootTable's yml file at ProgressivelyDifficultMobs/loottables/"));
                                    ProgressivelyMain.getInstance().getServer().getConsoleSender().sendMessage(Utils.chat("&cand replace the dropped item's type value with an alternative up-to-date variant."));
                                    ProgressivelyMain.getInstance().getServer().getConsoleSender().sendMessage(Utils.chat("&cExample: NETHERRITE_CHESTPLATE > DIAMOND_CHESTPLATE"));
                ...
                ProgressivelyMain.getInstance().getServer().broadcast(Utils.chat("Finished loading ProgressiveDifficulty: MOBS - Loot Tables"), "pdm.reload");
