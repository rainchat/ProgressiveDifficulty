...
import me.athlaeos.progressivelydifficultmobs.resourse.menus.LevelPerkModificationMenu;
        ...
        if (args.length <= 1) {
            ...
            if (args.length >= 3) {
                if (sender.hasPermission("pdm.manageperks")) {
                    if (ProgressivelyMain.getInstance().getServer().getPlayer(args[1]) != null) {
                        p = ProgressivelyMain.getInstance().getServer().getPlayer(args[1]);
                    } else {
